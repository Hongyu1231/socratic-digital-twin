...
There are five reasoning phases. A correct response advances the phase. Other responses receive another Socratic question. If two consecutive answers in the same phase are classified `wrong` with at least 85% confidence, the tutor states plainly that the claim is incorrect before asking the next question; `partial` and `vague` answers never trigger this rule. After the third unsuccessful attempt in a phase, the unresolved gap is recorded and the flow advances to avoid a loop. Completing phase five generates a summary automatically.
