...
左侧 **Case attachments** 提供与病例版本绑定的教学图片、音频或视频；未配置媒体时使用合成占位资源，文献图片可显示来源引用。点击麦克风可使用浏览器语音听写；Tutor 的 AI 生成语音回复默认开启，每条新回复到达后会自动朗读。顶部的 **Tutor voice** 可开关自动语音，消息旁的 **Read aloud** 可重播单条回复。OpenAI TTS 不可用时会自动退回设备英文语音；如果浏览器阻止自动播放，点击一次 **Read aloud** 即可授权播放。不应录入真实患者身份资料。
...
- 回答掌握当前目标时进入下一阶段。
- 未掌握时 tutor 会继续追问。
- 作答次数本身不会推进阶段；学生必须给出达到当前目标的正确推理，也可以选择提前结束。
- 同一阶段连续两次被判为高置信度 `wrong`（至少 85%）时，tutor 会先明确说明该陈述不正确，再继续一个苏格拉底式追问；`partial` 和 `vague` 永远不会触发该规则。
- 最终阶段会先进行反思性追问，完成后自动生成学习总结。
...
病例草稿最多可配置 12 个图片、音频或视频附件。媒体 URL 必须使用 HTTPS 或站内相对路径，并填写无诊断泄露的可访问描述；文献素材可附引用名称和来源 URL。已发布版本不可修改，调整媒体需点击 **New version** 创建新版本。只能使用合成、获得授权或正确去标识化的教学素材。
...
3. 创建包含 1–12 个阶段及病例影像附件的完整草稿，保存、发布，并演示 **New version**。
