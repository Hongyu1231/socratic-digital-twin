...
type HumanizationAction = "freeze" | "candidate" | "run" | "experiment" | "release" | "rollback";
...
export function FeedbackLab({ initialData, className = "" }: FeedbackLabProps) {
  ...
  const [datasetName, setDatasetName] = useState("Faculty review benchmark");
  ...
  const nextAction = !data.datasets.some((item) => item.status === "frozen")
    ? "Freeze professor-reviewed turns into a safe evaluation set."
    : !data.candidates.length
      ? "Create a candidate prompt or model version to compare."
      : !selectedRun
        ? "Run an offline comparison against the frozen evaluation set."
        : !selectedRun.gate?.passed
          ? "Review the comparison results and resolve any blocked safety gate."
          : !selectedApproval || selectedApproval.decision !== "approved"
            ? "Ask an independent professor to approve or reject the candidate."
            : !selectedRelease
              ? "Start shadow observation, then a limited A/B test before release."
              : "Monitor the active release and roll back if quality declines.";
  ...
  async function createCandidate(event: FormEvent<HTMLFormElement>) {
    ...
    const saved = await mutate("candidate", "/api/admin/humanization/candidates", { ...candidateDraft }, "Candidate saved. It is ready for offline evaluation.");
    if (saved) {
  ...
  async function freezeDataset() {
    await mutate("freeze", "/api/admin/humanization/datasets", { name: datasetName }, "De-identified faculty feedback was frozen into an immutable dataset.");
  }
  ...
  return (
    <section className={`grid gap-6 ${className}`} aria-labelledby="feedback-lab-title">
      <div className="overflow-hidden rounded-[3px_28px_3px_3px] bg-[#21172b] text-white shadow-[0_20px_55px_rgba(33,23,43,.16)]">
        <div className="grid gap-7 p-6 md:p-8 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)] xl:items-end">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-[.14em] text-[#f0c0b5]"><Sparkles size={13} /> Why this lab exists</span>
            <h3 id="feedback-lab-title" className="mt-4 max-w-4xl font-serif text-3xl leading-tight tracking-[-.03em] md:text-4xl">Use professor feedback to make the AI tutor more human—safely.</h3>
            <p className="mt-4 max-w-3xl text-sm leading-6 text-white/70">The lab tests whether a new prompt or model gives warmer, more Socratic and more accurate replies than the current tutor. It uses reviewed historical turns offline first, so an unproven change never reaches students.</p>
          </div>
          <div className="rounded-2xl border border-white/15 bg-white/10 p-5">
            <span className="text-[9px] font-extrabold uppercase tracking-[.14em] text-[#e7aca2]">Your next action</span>
            <p className="mt-2 font-serif text-lg leading-6">{nextAction}</p>
          </div>
        </div>
        <div className="grid border-t border-white/10 sm:grid-cols-3">
          <div className="p-5 sm:border-r sm:border-white/10"><span className="text-[9px] font-extrabold uppercase tracking-[.12em] text-[#e7aca2]">Input</span><strong className="mt-2 block font-serif text-base">Professor-reviewed turns</strong><p className="mt-1 text-[10px] leading-4 text-white/55">Labels and comments are de-identified before evaluation.</p></div>
          <div className="border-t border-white/10 p-5 sm:border-r sm:border-t-0 sm:border-white/10"><span className="text-[9px] font-extrabold uppercase tracking-[.12em] text-[#e7aca2]">Decision</span><strong className="mt-2 block font-serif text-base">Is the candidate better?</strong><p className="mt-1 text-[10px] leading-4 text-white/55">Compare accuracy, safety and human tutoring quality.</p></div>
          <div className="border-t border-white/10 p-5 sm:border-t-0"><span className="text-[9px] font-extrabold uppercase tracking-[.12em] text-[#e7aca2]">Output</span><strong className="mt-2 block font-serif text-base">Approved tutor release</strong><p className="mt-1 text-[10px] leading-4 text-white/55">Faculty approval is required, with rollback always available.</p></div>
        </div>
      </div>

      <section className={`${panelClass} p-5 md:p-6`} aria-labelledby="safe-path-heading">
        <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 shrink-0 text-[#476555]" size={20} /><div><span className="section-kicker">The safe improvement path</span><h3 id="safe-path-heading" className="mt-1 font-serif text-2xl text-[#21172b]">From professor feedback to a better tutor</h3></div></div>
        <div className="mt-5 grid gap-3 md:grid-cols-4">
          {[
            { number: "1", title: "Freeze evidence", text: "Create a de-identified, immutable set of professor-reviewed conversations." },
            { number: "2", title: "Compare offline", text: "Run the current tutor and candidate on the same cases, then compare quality metrics." },
            { number: "3", title: "Observe safely", text: "Use shadow mode first, followed by a small A/B test only after the gate passes." },
            { number: "4", title: "Faculty decides", text: "A professor approves the release; Admin can monitor or roll it back." },
          ].map((step) => <article key={step.number} className="rounded-xl border border-[#ded8d0] bg-[#f6f3ed] p-4"><span className="grid size-7 place-items-center rounded-full bg-[#de695c] text-[10px] font-extrabold text-white">{step.number}</span><strong className="mt-3 block font-serif text-base text-[#21172b]">{step.title}</strong><p className="mt-1 text-[10px] leading-4 text-[#726c73]">{step.text}</p></article>)}
        </div>
        <p className="mt-4 rounded-xl border border-[#b8d4c0] bg-[#e5ede7] p-3 text-[10px] leading-4 text-[#365846]"><strong>No online self-training:</strong> a single professor comment never changes the live prompt, model or student learning state automatically.</p>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(340px,.82fr)]">
        <div className="grid gap-6">
          <section className={`${panelClass} p-5 md:p-6`} aria-labelledby="dataset-heading">
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row"><input aria-label="Frozen dataset name" className={inputClass} value={datasetName} onChange={(event) => setDatasetName(event.target.value)} /><button type="button" className={`${buttonClass} shrink-0 bg-[#4e263f] text-white`} disabled={Boolean(busy) || datasetName.trim().length < 3} onClick={() => void freezeDataset()}>{busy === "freeze" ? <LoaderCircle size={14} className="spin" /> : <LockKeyhole size={14} />} {busy === "freeze" ? "Freezing…" : "Freeze reviewed turns"}</button></div>
            ...
            <div className="mt-5 grid gap-3">
              {data.datasets.map((dataset) => (
                <button key={dataset.id} type="button" onClick={() => setSelectedDatasetId(dataset.id)} className={`grid gap-2 rounded-xl border p-4 text-left transition sm:grid-cols-[minmax(0,1fr)_auto] ${dataset.id === selectedDatasetId ? "border-[#4e263f] bg-[#f6f3ed]" : "border-[#ded8d0] bg-white hover:border-[#de695c]"}`}>
                  <span className="min-w-0"><strong className="block truncate font-serif text-base text-[#21172b]">{displayDatasetName(dataset.name)}</strong><small className="mt-1 block text-[10px] text-[#726c73]">{dataset.entryCount} samples · hash {dataset.contentHash ? `${dataset.contentHash.slice(0, 10)}…` : "building"} · frozen {formatEnglishDate(dataset.frozenAt)}</small></span>
            ...
            {selectedDataset ? <div className="mt-4 grid gap-2 border-t border-[#ded8d0] pt-4 text-[10px] text-[#726c73] sm:grid-cols-3"><span>De-identification: <strong className="text-[#21172b]">{selectedDataset.deidentificationVersion}</strong></span><span>Source window: <strong className="text-[#21172b]">{formatEnglishDate(selectedDataset.sourceFrom)} – {formatEnglishDate(selectedDataset.sourceTo)}</strong></span><span>Created: <strong className="text-[#21172b]">{formatEnglishDate(selectedDataset.createdAt)}</strong></span></div> : null}
          </section>
        ...
        <aside className="grid content-start gap-6">
          <section className={`${panelClass} p-5 md:p-6`} aria-labelledby="run-heading">
            ...
            <div className="mt-5 grid gap-3"><label className="grid gap-1.5 text-xs font-bold text-[#4e263f]">Dataset<select className={inputClass} value={selectedDatasetId} onChange={(event) => setSelectedDatasetId(event.target.value)}><option value="">Select a frozen dataset</option>{data.datasets.map((item) => <option key={item.id} value={item.id}>{displayDatasetName(item.name)}</option>)}</select></label><label className="grid gap-1.5 text-xs font-bold text-[#4e263f]">Candidate<select className={inputClass} value={selectedCandidateId} onChange={(event) => setSelectedCandidateId(event.target.value)}><option value="">Select a candidate</option>{data.candidates.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.promptVersion}</option>)}</select></label></div>
            <button type="button" className={`${buttonClass} mt-4 w-full bg-[#4e263f] text-white`} disabled={!selectedDatasetId || !selectedCandidateId || Boolean(busy)} onClick={runEvaluation}>{busy === "run" ? <LoaderCircle size={14} className="spin" /> : <Play size={14} />} {busy === "run" ? "Running evaluation…" : "Run frozen-set evaluation"}</button>
            {data.runs.length ? <div className="mt-5 border-t border-[#ded8d0] pt-4"><label className="grid gap-1.5 text-xs font-bold text-[#4e263f]">Evaluation run<select className={inputClass} value={selectedRunId} onChange={(event) => setSelectedRunId(event.target.value)}>{data.runs.map((run) => <option key={run.id} value={run.id}>{run.status} · {formatEnglishDate(run.createdAt)}</option>)}</select></label></div> : null}
          </section>
