    ...
    <section className="case-resources" aria-labelledby={headingId}>
      ...
      <div className="resource-list">
        {attachments.map((attachment) => {
          ...
          const canPreview = attachment.kind !== "audio" && Boolean(attachment.url);
          const actionLabel = attachment.kind === "audio" ? playingId === attachment.id ? "Pause" : "Play" : "Open";
          return (
            <button
              className="resource-button"
              type="button"
              key={attachment.id}
              onClick={() => attachment.kind === "audio" ? toggleNarration(attachment) : canPreview ? setPreview(attachment) : undefined}
              disabled={attachment.kind === "audio" ? !speechAvailable || !attachment.transcript : !canPreview}
              aria-label={`${actionLabel} ${attachment.title}`}
            >
              <Icon size={15} />
              <span><strong>{attachment.title}</strong><small>{attachment.description}</small></span>
              {attachment.kind === "audio" ? playingId === attachment.id ? <Pause size={14} /> : <Play size={14} /> : <ExternalLink size={14} />}
            </button>
          );
        })}
      </div>
      ...
      {preview ? (
        <div className="media-dialog-backdrop">
          <div className="media-dialog" role="dialog" aria-modal="true" aria-labelledby="media-dialog-title">
            ...
            {preview.kind === "image" && preview.url ? <Image src={preview.url} alt={preview.description} width={1200} height={760} /> : null}
